...
@click.option('--seed',                   help='Random seed', type=int, default=1989, show_default=True)
...
def run_projection(
    ...
    # Load target image.
    target_pil = PIL.Image.open(target_image_fname).convert('RGB')
    w, h = target_pil.size
    s = min(w, h)
    ...
    if save_video:
        ...
        for i, projected_w in enumerate(projected_w_steps):
            if i % steps_per_frame == 0:
                ...
                rgb_video.append_data(np.concatenate([target_uint8, normalize_rgb(synth_result['image'])], axis=1))
