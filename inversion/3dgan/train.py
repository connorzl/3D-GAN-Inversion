...
@click.option('--cfg',          help='Base configuration',                                      type=click.Choice(['triplane', 'triplane_cars', 'triplane_no_dd', 'triplane_mod']), required=True)
...
@click.option('--neural_rendering_resolution_initial', help='Resolution to render at', metavar='INT',  type=click.IntRange(min=1), required=True)
...
@click.option('--swapping_prob_final', help='Strength of swapping regularization. -1 means condition with zeros.', metavar='FLOAT',  type=click.FloatRange(min=-1), required=False, default=-1)
...
@click.option('--real_downsample_mode', help='Type of downsample for raw/lores', metavar='STR',  type=click.Choice(['filtered', 'bilinear']), required=False, default='filtered')
...
@click.option('--gen_module',    help='Generator module', metavar='STR',  type=str, required=False)
...
def main(**kwargs):
    ...
    if opts.cfg == 'triplane':
        c.G_kwargs.class_name = 'training.triplane.SRPosedGenerator'
        c.D_kwargs.class_name = 'training.networks_stylegan2.DualDiscriminator'
        c.G_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions.
        rendering_options = {
            ...
            'box_warp': 1,
            'clamp_mode': 'softplus',
            'superresolution_module': opts.sr_module,
            'c_gen_conditioning_zero': (opts.swapping_prob_final == -1),
        ...
        c.loss_kwargs.swapping_prob_final = opts.swapping_prob_final
        ...
        c.loss_kwargs.real_downsample_mode = opts.real_downsample_mode

        if 'AF' in opts.sr_module:
            c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax)
            c.G_kwargs.sr_kwargs.sr_first_cutoff = opts.sr_first_cutoff
            c.G_kwargs.sr_kwargs.sr_first_stopband = opts.sr_first_stopband
        else:
    ...
    elif opts.cfg == 'triplane_no_dd':
        c.G_kwargs.class_name = 'training.triplane.SRPosedGenerator'
        c.D_kwargs.class_name = 'training.networks_stylegan2.DummyDualDiscriminator'
        c.G_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions.
        rendering_options = {
            'image_resolution': c.training_set_kwargs.resolution,
        ...
        c.loss_kwargs.style_mixing_prob = opts.style_mixing_prob
    elif opts.cfg == 'triplane_mod':
        c.G_kwargs.class_name = opts.gen_module#'training.triplane_double_layer.TriPlaneGenerator'
        c.D_kwargs.class_name = 'training.networks_stylegan2.DualDiscriminator'
        c.G_kwargs.fused_modconv_default = 'inference_only' # Speed up training by using regular convolutions instead of grouped convolutions.
        rendering_options = {
            'image_resolution': c.training_set_kwargs.resolution,
